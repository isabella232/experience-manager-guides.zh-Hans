...
除了预定义字段外，您还可以将元数据信息作为变量或字段添加到页面布局中。 此元数据存储在您的源DITA中 **映射内容**，也可以从DITA中选取 **映射文件属性** 或 **主题文件属性** 并轻松插入页面布局。

您可以从以下选项中选择元数据：

* **映射内容** 包含您在中定义的元数据 `<topicmeta>` DITA映射的元素。
* **映射文件属性** 包括元数据，您可以从 **属性** DITA映射的页面。
* **主题文件属性** 包括元数据，您可以从 **属性** 主题页面。


您可以从以下来源合并元数据 **映射文件属性** 和 **主题文件属性** 在一个文档中。 例如，您可以发布一个PDF，其封面页具有地图标题，而其他页面标题具有主题标题。 为此，您可以添加地图标题元数据，该元数据来自 **映射文件属性** 到封面布局。 然后，从添加主题标题元数据 **主题文件属性**  到章节和主题页面布局的标题。

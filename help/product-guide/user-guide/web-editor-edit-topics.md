...
Experience Manager指南还允许您跨元素选择内容。 选择内容后，您可以执行以下操作：
...
- 使用有效元素括起内容：执行以下步骤以使用有效元素括起内容：
   - 选择元素中的内容。
   - 选择 ![添加](images/Add_icon.svg) 图标，以查看 **用元素括起来** 对话框。 该对话框列出了选定内容的有效元素。
     >[!NOTE]
     >
     > 也可以通过选择所选内容的上下文菜单来查看“带元素的环绕”对话框。

   - 从对话框中选择元素。 选定的内容将封装在该元素下。 例如，如果您选择段落中的内容，然后选择 `<note>` 元素来自 **用元素括起来** 对话框，选定内容会显示在注释下。\
     ![“环绕元素”对话框](./images/surround-element.png) {width="300" align="left"}

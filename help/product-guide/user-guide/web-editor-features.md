...
- **翻译**：此选项卡包含创建语言组、将源标签传播到目标版本以及清理翻译项目的选项。
  ![](images/editor-setting-translation.png){width="550" align="left"}

   - **语言组**：作为管理员，您可以创建一组语言，并将它们作为一组用于翻译内容。\
     执行以下步骤以创建新语言组：
      1. 选择添加 ![“添加”图标](images/Add_icon.svg) 图标。
     ...
     键入语言的前几个字符或语言代码以筛选所需的语言。 例如，键入“en”可筛选名称或代码开头包含“en”的所有语言。
      1. 选择 **完成** 将所选语言添加到组。 将显示语言。 当你添加三种或多种语言时， **显示更多** 显示。 您可以选择 **显示更多** 查看组中存在的所有语言。
         >[!TIP]
         >
         > 切换 **显示更多** 到 **显示更少** 只查看几种语言。

      1. 将鼠标悬停在要编辑的组中的语言上 ![编辑图标](images/edit_pencil_icon.svg) 或删除 ![删除](images/Delete_icon.svg) 语言组。
      1. 保存 **编辑器设置**.
   ...
   - **翻译项目完成后清理**：选择此选项可将翻译项目配置为在翻译后自动禁用或删除。 默认情况下， **无** 处于选中状态，因此项目在翻译后存在。
...
**用户首选项** - ![用户偏好设置图标](images/user_preference_editor_icon.svg)
...
- **外观**：选择Web编辑器应用程序的主题和内容编辑区域的源视图。
  ...
  ![用户首选项的外观选项卡](images/user_preference_editor_appearance.png){width="550" align="left"}
   ...
   - **应用程序主题**：您可以从 **浅色** 或 **深色** 应用程序的主题。 在 **浅色** 主题，工具栏和面板使用浅灰色背景色。 在 **深色** 主题，工具栏和面板使用黑色背景。 选择 **使用设备主题** 允许Experience Manager指南根据设备主题选择浅色和深色主题。  在所有主题中，内容编辑区域都以白色背景显示在 **作者** 视图。

   - **源视图主题**： — 您可以从 **浅色** 或 **深色** 源视图中内容编辑区域的主题。 在 **浅色** 主题，在的情况下，内容编辑区域使用浅灰色背景作为源视图 **深色** 主题，它使用黑色背景。 选择 **使用设备主题** 允许Experience Manager指南根据设备主题选择浅色和深色主题。

   - **始终在存储库中查找文件**：选择此选项可在Web编辑器中编辑文件时显示文件在存储库中的位置。

   - **在创作模式下显示不间断空间指示器**：选择此选项可在Web编辑器中编辑时显示不间断空格的指示符。 默认情况下处于启用状态。
...
在当前或下一个有效位置插入有效元素。 您也可以使用键盘快捷键 ***Alt***+***输入*** 以打开“插入元素”弹出菜单。 例如，您正在编辑一个段落，然后在 **插入元素** 弹出窗口，将显示可以插入段落中的元素列表。 选择要插入的元素。 您可以使用键盘滚动元素列表并按 ***输入*** 以插入所需的元素。

您可以查看两种类型的有效元素：

- **当前位置的有效元素**：列表显示可在当前光标位置本身插入的元素。

- **当前位置之外的有效元素**：列表显示了可以在元素层次结构中的当前元素的任何父元素之后插入的元素。



例如，如果您位于内联 `<b>` 元素，您可以插入如下元素 `<u>`， `<xref>`， `<i>` 在当前位置。 相反，您可以插入以下元素 `<table>` 和 `<topic>` 位于当前位置之外。
...
*应用筛选器以搜索包含文本的文件`general purpose.`*

选择 **筛选搜索** \(![“搜索筛选器”图标](images/filter-search-icon.svg)\)图标，打开筛选依据弹出窗口。

>[!NOTE]
>
> 当您搜索任何文本或过滤任何文件时， **筛选搜索**  \(![“搜索筛选器”图标](images/filter-search-icon.svg)\)图标，指示我们位于“搜索”面板上，并且已应用某些过滤器。


您可以使用以下选项来筛选文件，并在Adobe Experience Manager存储库中缩小搜索范围：

- **DITA文件**：您可以查找所有 **DITA主题** 和 **DITA映射** 在选定路径中存在。 默认情况下，将选择这些选项。
- **非DITA文件**：您可以搜索 **Ditaval文件**，  **图像文件**， **多媒体**， **文档**、和 **Json** 在选定路径中。
...
选择 **高级筛选** ![“高级过滤器”图标](images/advanced-filter-gear-icon.svg)图标以查看 **高级过滤器** 对话框。

您可以在 **常规** 和 **高级** 选项卡。

![高级过滤器对话框](images/repository-filter-search-advanced.png) {width="800" align="left"}


**常规**

- **搜索结果将如下所示**：在Adobe Experience Manager存储库选定路径上的文件中搜索某些文本。 在文件的标题、文件名和内容中搜索文本。

这与存储库窗口中的搜索框同步。 例如，如果您键入 `general purpose` 在“存储库”面板上的搜索框中，它还会出现在 **高级过滤器** 对话框，反之亦然。

- **搜索位置**：选择用于搜索Adobe Experience Manager存储库中文件所在位置的路径。
...
- **DITA元素**：还可以在指定DITA元素的属性中搜索特定值。
   - 选择 **添加元素** ![“添加”图标](images/Add_icon.svg) 以添加元素、属性和值。
   - 应用已选择的过滤器。

- 选择 **全部清除** 以清除所有应用的过滤器。


- 选择 **关闭筛选器** ![关闭图标](images/close-icon.svg) 图标以关闭过滤器并返回到存储库的树视图。
  >[!NOTE]
  >
  >系统管理员还可以配置文本过滤器，并显示或隐藏其他过滤器。 有关更多详细信息，请参阅 *配置文本过滤器* 安装和配置Adobe Experience Manager Guidesas a Cloud Service中的部分。

  将显示包含搜索文本的已过滤文件的列表。 例如，包含文本的文件 `general purpose` 列在上一个屏幕快照中。 您可以从筛选列表中选择多个文件以将其拖放到打开进行编辑的映射中。
...
- **上传资产**：将文件从本地系统上传到Adobe Experience Manager存储库中的选定文件夹。 您还可以将文件从本地系统拖放到当前工作主题上。 如果要将本地系统中的图像插入主题，这非常有用。
...
**内容属性** -  ![内容属性](images/content-properties-icon.svg)

您可以访问 **内容属性** 功能，方法是选择 **内容属性** 图标。 此 **内容属性** 面板包含有关文档中当前选定元素的类型及其属性的信息。

**类型**：您可以查看并从下拉列表中为当前标记选择完整层次结构的标记。

**属性**：和 **属性** “布局”、“创作”和“源”视图中提供了下拉面板。 您可以轻松地添加、编辑或删除属性。

...
1. 将更新后的ZIP文件上传回AEM存储库。
   ...
   >[!NOTE]
   >
   > 建议不要覆盖默认的DITA-OT包。 您应该上载包含插件/var/dxml/dita\_resources/dita-ot文件夹的自定义DITA-OT包。 您还可以使用Cloud Manager管道执行此操作，请参阅 [部署到AEMas a Cloud Service](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/overview.html) 有关详细信息，请参阅AEM文档。
...
1. 配置以下属性以使用自定义DITA-OT插件：
   ...
   | DITA-OT环境变量 | *\（可选\）* 指定要传递到DITA-OT进程的环境变量。 默认情况下，AEM Guides会添加四个变量 —  `ANT_OPTS`， `ANT_HOME`， `PATH`、和 `CLASSPATH`. <br> 您可以重用任何现有系统环境变量或属性来构建新环境变量。 例如，如果您拥有 `JAVA_HOME` 系统中定义的系统变量，并且您想要定义一个名为的新环境变量 `JAVA_BIN` 这是使用以下项目构建的： `JAVA_HOME`. 然后，您可以添加 `JAVA_BIN` 作为：`JAVA_BIN= ${JAVA_HOME}/bin` <br> **注意：** 您还可以使用Java系统属性来构建环境变量。 例如，如果AEM启动脚本定义了Java系统属性 `java.io.tmpdir` 对于临时目录，可以使用此属性将新变量定义为： `${java.io.tmpdir}/fmdita/dita_ot`. <br> **重要提示：** 要重用任何现有的系统变量或资产，必须将它包含在 `${}`. |
   ...
   | DITA-OT临时路径 | 复制DITA文件以进行处理的临时位置。 在DITA-OT处理文件之前，这些文件将复制到此临时位置。 默认情况下，临时存储位置为： <br> `<*AEM-Install*>/crx-quickstart/profiles/ditamaps` <br> **重要提示：** 您不能更改默认路径。 |

   >[!NOTE]
   >
   > AEM Guides安装程序会创建两个环境变量，您可以使用这两个环境变量指定自定义DITA-OT插件文件的路径。 这些环境变量包括： `DITAOT\_DIR`，其中包含文件系统中DITA-OT目录的路径；以及 `DITAMAP\_DIR`，其中包含文件系统中提取DITA映射内容的路径。
...
执行以下步骤以创建新配置文件并将其配置为在AEM Guides中使用专用的DTD和XSD：
